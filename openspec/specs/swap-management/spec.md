...
## Purpose
Defines the behavior of the swap management provisioning stage, which disables swap at runtime, comments out swap entries in `/etc/fstab` to persist the change across reboots, and supports safe rollback. Required because the kubelet refuses to start on hosts with active swap.
