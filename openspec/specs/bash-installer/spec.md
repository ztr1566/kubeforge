...
## Purpose
Defines the behavior of the standalone bash installer script that downloads and installs the `k8s-ready` binary from GitHub Releases onto a Linux host. The installer enforces root privileges, detects the host architecture, verifies dependencies, fetches the correct release asset, installs the binary globally, and verifies the installation.
...
#### Scenario: Installer run at root
