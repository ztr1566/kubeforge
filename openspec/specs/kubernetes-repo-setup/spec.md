...
## Purpose
Defines the behavior of the Kubernetes repository setup provisioning stage, which configures the host's apt sources to use the modern `pkgs.k8s.io` repository for the user-selected Kubernetes minor version. Creates `/etc/apt/keyrings/`, downloads and dearmors the repository GPG signing key, and writes `/etc/apt/sources.list.d/kubernetes.list`.
