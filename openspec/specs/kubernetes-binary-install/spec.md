...
## Purpose
Defines the behavior of the Kubernetes binary install provisioning stage, which refreshes the apt package index, installs `kubelet`, `kubeadm`, and `kubectl` pinned to the exact user-selected version, holds the packages against automatic upgrades, and enables/starts the kubelet systemd service.
