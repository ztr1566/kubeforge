# Flannel Network Prep

## Purpose
Defines the behavior of the Flannel network preparation provisioning stage, which prepares the host for the Flannel CNI plugin by emitting the `kubeadm init` guidance with the Flannel-compatible pod CIDR and writing a kubeadm init configuration snippet on master nodes. Ensures strict compatibility with Flannel's default `10.244.0.0/16` pod network.
